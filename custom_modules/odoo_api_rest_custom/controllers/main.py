...
def validate_token(f):
    ...
    def wrap(*args, **kwargs):
        try:
            ...
            access_token_data = (
                request.env["apirest.store.details"].sudo().search([("x_api_token", "=", access_token)],
            ...
            user_token = access_token_data.find_user(username=access_token_data.x_api_username,
                                                     password=access_token_data.x_api_password)
            if not user_token:
                Response.status = "401 unauthorized"
                return {'code': 401, 'error': 'El usuario no esta registrado o no existe, verifique los datos.',
                        'message': 'error'}
...
class ApiAccess(Controller):
    ...
    @validate_token
    @route('/api/web/get_products/<int:user_id>', type='json', auth='none', methods=['GET'], website=False, csrf=False)
