    ...
    PickingClientAction.include({
        _makeNewLine: function (product, barcode, qty_done, package_id, result_package_id) {
            ...
            var newLine = {
                ...
                'location_dest_id': {
                    'id': (product.x_location != undefined) ? product.x_location[0] : currentPage.location_dest_id,
                    'display_name': (product.x_location != undefined) ? product.x_location[1] : currentPage.location_dest_name,
                },
                'package_id': package_id,
                'result_package_id':  (product.x_package != undefined) ? [product.x_package[0], product.x_package[1]] : result_package_id,
                'state': 'assigned',
                'reference': this.name,
